## [424 Longest Repeating Character Replacement](https://leetcode.com/problems/longest-repeating-character-replacement/description/)

<!-- notecardId: 1763061949000 -->

```js
// Explanation:
// - Neetcode: https://youtu.be/gqXU1UyA8pk

// Sub-pattern:
// - Lazy Tracking & Variable-size window

// - Time: O(n)
// - Space: O(1)
function characterReplacement(s, k) {
  const ALPHABET_SIZE = 26;
  const charFreq = new Uint32Array(ALPHABET_SIZE);
  const codeA = 'A'.codePointAt(0);
  let mostFreq = 0;
  let longest = 0;

  let left = 0;

  for (let right = 0; right < s.length; right += 1) {
    const index = s.codePointAt(right) - codeA;
    charFreq[index] += 1;
    mostFreq = Math.max(mostFreq, charFreq[index]);
    const windowSize = right - left + 1;

    if (windowSize - mostFreq <= k) {
      longest = Math.max(longest, windowSize);
    } else {
      charFreq[s.codePointAt(left) - codeA] -= 1;
      left += 1;
    }
  }

  return longest;
}

// 🧩 Here's the intuition:
// We have a window of size windowSize = 5 and k = 1 (so we can replace at most 1 character).
//
// The window is valid if:
// windowSize - mostFreq <= k
// where mostFreq is the count of the most frequent character in the window.
//
// Scenario 1: everything fits
// windowSize = 5
// mostFreq = 4  // e.g., A A A A B
// 5 - 4 = 1 <= 1 → valid
// So we can keep the window, longest = 5
//
// Scenario 2: window slides
// Suppose we remove one 'A', now it's A A A B B
// Real max frequency = 3, but mostFreq in code is still 4
//
// windowSize = 5
// mostFreq = 4 (overcounted)
// 5 - 4 = 1 <= 1 → still "valid" in the code
//
// But in reality:
// 5 - 3 = 2 > 1 → not valid
//
// You might think: "That's wrong!" — but it's not.
//
// Why? Because we don't update longest here!
// The algorithm only updates longest when windowSize - mostFreq <= k.
// And the length 5 was already recorded earlier when the window was really valid.
// So even if the window now is "falsely valid", it won't give a result bigger than 5,
// which means the answer stays correct.
//
// This approach is called Lazy Tracking:
// we don't decrease mostFreq when the window shrinks, because overcounting it
// doesn't break correctness and avoids extra computation.
```

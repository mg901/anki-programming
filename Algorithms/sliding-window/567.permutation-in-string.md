...
function checkInclusion(s1, s2) {
  ...
  for (let i = 0; i < s1.length; i += 1) {
    s1Freq[s1.charCodeAt(i) - base] += 1;
    s2Freq[s2.charCodeAt(i) - base] += 1;
  }
  ...
  for (let i = s1.length; i < s2.length; i += 1) {
    ...
    const rightIndex = s2.charCodeAt(i) - base;
    const leftIndex = s2.charCodeAt(i - s1.length) - base;
